...
contract Contract {
    ...
    struct Fine {
        ...
        string expiryDateFine;    // Срок оплаты штрафа
    }
    ...
    function calculationInsurance(uint256 _carIndex) public view returns (uint256) {
        ...
        uint256 Rs = Cars[msg.sender][_carIndex].marketValue;
        uint256 Ce = Cars[msg.sender][_carIndex].serviceLife;
        uint256 Shtr = Drivers[msg.sender].numberUnpaidFines;
        uint256 Dtp = Drivers[msg.sender].numberAccident;
        uint256 Vs = Drivers[msg.sender].drivingExperience;

        uint256 strVzn = (Rs * (10 - Ce / 100) + 2 * Shtr + Dtp - 2 * Vs) / 10;
